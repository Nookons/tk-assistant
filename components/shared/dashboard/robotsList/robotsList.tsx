...
const RobotsList = ({card_id}: { card_id: ParamValue }) => {
    ...
    return (
        <div className="border rounded-b-xl mb-4 p-2">
            <div className={`md:flex md:justify-between grid gap-4 my-2 px-1`}>
                {/*<div>
                    ...
                    {filtered_data.length > 0 &&
                        <div className={`flex flex-wrap gap-2`}>
                            {filtered_data.slice(0, 5).map((item, i) => (
                                <Item variant={`muted`} className={`p-2 w-full`}>
                                    <div className={`grid gap-2 grid-cols-[80px_1fr_1fr] w-full`}>
