...
import {TrendingDown, TrendingUp, Minus, WrenchIcon, Bug} from "lucide-react";
...
const ExceptionsStats = () => {
    ...
    const getHistoryData = async () => {
        try {
            const response = await ExceptionService.getExceptionsHistory(session?.warehouse_sessions ?? "");

            if (response) {
                setStats({currentMonth: response.currentMonth, prevMonth: response.prevMonth});
            }
        } catch (error) {
            console.error(error);
        ...
        <Card>
            <CardHeader className="pb-3">
                ...
                <p className="text-xs text-muted-foreground">
                    Robots got issues in {currentMonthLabel}
                </p>
                <p>
...
export default ExceptionsStats;